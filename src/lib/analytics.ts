...
export function computeMonthAnalytics(
  ...
  const categoryBudgets: CategoryStat[] = Object.entries(budgets)
    ...
    .map(([category, budget]) => {
      ...
      return {
        ...
        budget,
        expected,
        projected: daysElapsed > 0 ? (total / daysElapsed) * daysInPeriod : total,
  ...
  return {
    ...
    heatmap,
    daily,
    todayEntries: todayEntries.sort(
...
export function computeYearAnalytics(
  ...
  const daysElapsed = isCurrentYear ? differenceInCalendarDays(now, yearStart) + 1 : daysInPeriod
