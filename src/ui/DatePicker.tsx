...
export function DatePicker({
  ...
  const padStart = getDay(monthStart); // 0 = Sunday
  ...
  // Determine if the pill is active (value is set and not matching any external preset logic)
  const pillIsActive = active ?? (variant === "pill" && !!value);

  return (
    <div ref={containerRef} className={`relative ${variant === "input" ? "w-full" : ""}`}>
      {/* Trigger */}
      {variant === "pill" ? (
        <button
          type="button"
      ...
      {open && (
        ...
        >
          ...
          <div className="flex items-center justify-between mb-3 px-1">
            ...
            <button
              type="button"
              onClick={() => setOpen(false) /* click month label to close */}
