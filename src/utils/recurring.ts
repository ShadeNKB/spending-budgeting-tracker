...
export function detectRecurring(expenses: Expense[]): RecurringGroup[] {
  ...
  for (const [key, items] of merged.entries()) {
    ...
    const avgAmount = items.reduce((s, e) => s + e.amount, 0) / items.length
 ...
 */
export function forecastDailyFromRecurring(groups: RecurringGroup[], days: number): number[] {
