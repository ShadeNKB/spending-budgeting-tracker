...
// - Budget v4.0: per-key LWW via `budgetUpdatedAt` / `budgetDeletedAt` timestamps.
...
export function mergeBackups(local: BackupData, remote: BackupData): BackupData {
  ...
  for (const key of allBudgetKeys) {
    ...
    const localSetTs = ts(
      local.budgetUpdatedAt?.[key] ??
        (local.budgets?.[key] !== undefined ? local.exportDate : undefined),
    )
    const remoteSetTs = ts(
      remote.budgetUpdatedAt?.[key] ??
        (remote.budgets?.[key] !== undefined ? remote.exportDate : undefined),
    )
...
export async function subscribeSync(syncId: string, onUpdate: () => void): Promise<() => void> {
  ...
  const open = () => {
    ...
    channel = client
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: TABLE, filter: `sync_id=eq.${syncId}` },
        () => {
          if (active) onUpdate()
        },
      )
